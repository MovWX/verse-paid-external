...
namespace render {
	...
	public:
		...
		auto hijack() -> bool {
			...
			if (!window_handle)
			{
				MessageBoxA(0, skCrypt("Medal Not Found | Retry"), skCrypt(""), MB_ICONINFORMATION);
			}
		...
		auto Menu() -> void {
			ImGui::SetNextWindowSize(ImVec2(418, 250));
			...
			ImGui::Begin(skCrypt("hus X veru"), (bool*)true, window_flags);
			{
				...
				ImGui::GetWindowDrawList()->AddRectFilled(window_pos, { window_pos.x + window_current.x, window_pos.y + 65 }, ImColor(25, 25, 25), 3.f, ImDrawCornerFlags_Top);
				...
				if (tab == 0)
				{
					ImGui::BeginChild(("Aimbot"), { 450 / 2 - 20, 280 });
					{
						ImGui::BeginGroup();
						{
							ImGui::SetCursorPos({ 0, 5 });
							Custom1337::Checkbox(("Aimbot"), &globals->aimbot);
							Custom1337::Checkbox(("Triggerbot"), &globals->triggerbot);
					...
					ImGui::EndChild();
					ImGui::SameLine();
					ImGui::BeginChild(("Settings"), { 450 / 2 - 20, 280 });
					{
						ImGui::SetCursorPos({ 0, 5 });
						ImGui::BeginGroup();
						{
							...
							ImGui::Spacing();
							HotkeyButtontrig(globals->triggerbot_key, ChangeKeytrig, keystatustrig);
						}
						ImGui::EndGroup();
					}
					ImGui::EndChild();
				...
				if (tab == 2)
				{
					ImGui::BeginChild(("Visuals"), { 450 / 2 - 20, 280 });
					{
						ImGui::BeginGroup();
						{
							ImGui::SetCursorPos({ 0, 5 });
							if (Custom1337::Checkbox(("Box"), &globals->bounding_box)) {
								globals->cornerbox = 0;
							}

							Custom1337::Checkbox(("Skeleton"), &globals->skeleton);

							if (Custom1337::Checkbox(("Corner Box"), &globals->cornerbox)) {
								globals->bounding_box = 0;
							}
		...
		auto Draw() -> void {			
			...
			if (globals->render_field_of_view) {
				...
				{
					ImGui::GetBackgroundDrawList()->AddCircle(ImVec2(globals->width / 2, globals->height / 2), globals->field_of_view, ImColor(255, 0, 255, 255), 33, 1);
				}
			}
